...
>
> Nếu OLED hiện `ESP32 OFFLINE` thì nút `ENROLL` sẽ không bắt đầu đăng ký cho tới khi kiểm tra lại dây UART/camera và ESP32 gửi `READY`.
...
> Nếu OLED hiện `ESP32 OFFLINE` thì nút `DELETE` sẽ không xóa dữ liệu vì STM32 chưa nhận được phản hồi sẵn sàng từ ESP32.
