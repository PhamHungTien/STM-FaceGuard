...
- **Nhận diện khuôn mặt thời gian thực** — ESP32-S3 xử lý AI với camera OV3660 3MP, cosine similarity ≥ 0.40
...
- **Đèn báo đã thấy mặt** — ESP32-S3 bật đèn trắng để báo người dùng giữ nguyên vị trí khi camera đã bám được khuôn mặt
...
- **Watchdog kép** — IWDG (STM32, ~5.0s) luôn bật; Task WDT (ESP32, 30s) dùng cho firmware tích hợp và có thể tắt riêng ở chế độ standalone debug
...
> - Firmware hiện dùng **GPIO47** như đèn báo khi đã thấy khuôn mặt để người dùng giữ nguyên tư thế; kênh RGB/NeoPixel **GPIO48** sáng xanh khi camera sẵn sàng và sáng trắng tối đa khi detect được khuôn mặt
...
| GPIO47 | Đèn báo trắng | Bật ngắn khi phát hiện khuôn mặt |
| GPIO48 | RGB/NeoPixel phụ | Xanh khi camera sẵn sàng, trắng tối đa khi detect được khuôn mặt |
...
> **Lưu ý:**
> - Nếu không phát hiện khuôn mặt trong 12 giây, OLED nhắc lại bước hiện tại
...
| `SYS_ENROLLING` (step) | `Step 1/1 / Look at camera / (O)` | Hướng dẫn đăng ký trong cấu hình 1 tư thế hiện tại |
...
Firmware hiện tại đang đặt `REQUIRED_MATCHES = 1`, tức chỉ cần **1 frame hợp lệ** với similarity vượt ngưỡng là ESP32 có thể gửi `OPEN`. Phần đăng ký cũng đang để `ENROLL_TOTAL_STEPS = 1`, nên chỉ cần **1 frame mặt thẳng** là có thể enroll xong. Cấu hình này ưu tiên độ trễ thấp và thao tác nhanh; nếu môi trường ánh sáng phức tạp hoặc có nguy cơ false positive thì nên tăng lại số frame hoặc số tư thế.
...
|---------|--------|------------|
| `RECOGNITION_THRESHOLD` | 0.40 | Tăng → an toàn hơn nhưng có thể từ chối người hợp lệ trong điều kiện ánh sáng xấu |
| `REQUIRED_MATCHES` | 1 | Tăng → chậm hơn nhưng ít false positive hơn |
| `ENROLL_TOTAL_STEPS` | 1 | Tăng → enroll chậm hơn nhưng dữ liệu đa góc ổn định hơn |

### 3a. Tuning detector để bắt mặt ở xa hơn

Firmware hiện tại đã nới detector theo hướng ưu tiên bắt được khuôn mặt nhỏ hơn trong khung hình:

| Detector | Cấu hình hiện tại | Ý nghĩa |
|---------|-------------------|---------|
| `HumanFaceDetectMSR01` | `score=0.08`, `top_k=16`, `resize_scale=0.20` | Giữ lại nhiều candidate hơn từ stage coarse detector |
| `HumanFaceDetectMNP01` | `score=0.40`, `top_k=8` | Nới stage refine để bớt bỏ sót khuôn mặt ở xa |

Các thay đổi này giúp bắt mặt ở xa tốt hơn trong giới hạn hiện tại của cảm biến và frame `240x240`, nhưng cũng có thể làm tăng nhẹ false positive. Nếu cần an toàn hơn, tăng lại `score_threshold` hoặc `RECOGNITION_THRESHOLD`.
...
| Task WDT | ESP32-S3 | 30 giây | Dùng ở firmware tích hợp; nhánh standalone debug hiện có thể tắt để chẩn đoán panic/runtime dễ hơn |
...
### Cấu hình đã test ổn định bằng `arduino-cli`

Trong nhánh debug hiện tại, cấu hình dưới đây đã compile, upload và boot ổn định trên board thật:

```bash
arduino-cli compile \
  --fqbn 'esp32:esp32:esp32s3:USBMode=hwcdc,CDCOnBoot=default,UploadMode=default,CPUFreq=240,FlashMode=dio,FlashSize=16M,PartitionScheme=huge_app,DebugLevel=none,PSRAM=opi,LoopCore=1,EventsCore=1,EraseFlash=none,JTAGAdapter=default,UploadSpeed=921600' \
...
- Nhánh AI và nhánh preview hiện đã được tách riêng: `vision_task` xử lý detect/recognize, còn HTTP preview chỉ chụp snapshot khi có request.
- Camera được bảo vệ bởi mutex, nên preview và face AI không còn giành cùng framebuffer cùng lúc.
- Nội bộ firmware dùng luồng `RGB565 -> face detect`, và chỉ chuyển sang `RGB888` khi thật sự cần enroll/recognize; cách này ổn định hơn so với ép mọi frame sang `RGB888` ngay từ đầu.
- Firmware hiện dùng preset low-light rõ ràng hơn cho OV3660: tăng `brightness`, dùng `manual exposure/gain` ở mức indoor và tăng `denoise` để ảnh gốc sáng hơn trong phòng tối.
- Với cách gắn camera ngang hiện tại, firmware xoay cả preview web lẫn pipeline detect sang **phải 90°**, đồng thời giữ trục mirror tự nhiên như selfie.
- Ảnh preview được nâng sáng sau khi đã đổi sang `RGB888`, rồi mới nén JPEG; cách này giảm rõ rệt artifact màu so với hậu xử lý trực tiếp trên `RGB565`.
- Web preview hiện hiển thị luôn khung detect của khuôn mặt mới nhất bằng overlay HTML/CSS, nên có thể nhìn trực tiếp vùng mặt mà AI đang bám theo.
- `GPIO47` chỉ còn đóng vai trò đèn báo “đã thấy khuôn mặt”, còn RGB `GPIO48` sáng xanh khi camera ready và sáng trắng tối đa khi detect mặt. Nếu board thực tế khác biến thể này, sửa các hằng `FACE_LIGHT_*` trong [STM-FaceGuard.ino](/Users/phamhungtien/Documents/STM-FaceGuard/ESP32-S3/STM-FaceGuard/STM-FaceGuard.ino#L94).
...
| Ngưỡng nhận diện (similarity) | **≥ 0.40** (cosine similarity) |
...
| Số tư thế đăng ký | **1 tư thế** trong cấu hình hiện tại |
| Thời gian giữ mỗi tư thế đăng ký | 2.0 giây |
| Timeout đăng ký | 15 giây/lần chờ step (STM32, có retry ENROLL tối đa 2 lần) / 12 giây/bước (ESP32) |
...
| Watchdog ESP32 (Task WDT) | 30 giây trong firmware tích hợp; có thể tắt ở nhánh standalone debug |
