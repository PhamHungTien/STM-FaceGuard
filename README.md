...
> Code đã cấu hình **PULLUP + FALLING edge** cho `PC13`, và `PA0/PA1` là **GPIO input pull-up** (đọc polling + debounce) trong `MX_GPIO_Init()`.
>
...
|-----------|-----------------|----------|-------------|
| **PA9** | CN10 – D8 | USART1 TX | ESP32-S3 **GPIO20** (RX) |
| **PA10** | CN10 – D2 | USART1 RX | ESP32-S3 **GPIO19** (TX) |
...
| **PA0** | Theo chân MCU `PA0` trên board | GPIO input (polling) | Nút **ENROLL** → GND |
| **PA1** | Theo chân MCU `PA1` trên board | GPIO input (polling) | Nút **DELETE** → GND |
...
ESP32-S3 yêu cầu **2 frame liên tiếp** cùng nhận ra một Face ID với similarity ≥ 0.55 trước khi gửi `OPEN`. Nếu frame thứ 2 nhận ra ID khác → reset bộ đếm.
...
|---------|--------|------------|
| `RECOGNITION_THRESHOLD` | 0.55 | Tăng → an toàn hơn nhưng có thể từ chối người hợp lệ trong điều kiện ánh sáng xấu |
...
|---------|--------------|---------|----------|
| IWDG (Independent) | STM32F303RE | ~5.0 giây | Reset STM32 nếu main loop bị treo |
| Task WDT | ESP32-S3 | 30 giây | Panic + reset ESP32 nếu loop() bị treo |

> IWDG STM32 sử dụng LSI nội (xấp xỉ 40 kHz, có sai số theo chip) — hoạt động ngay cả khi clock chính bị lỗi.
...
- Firmware hiện bật đèn trắng chính trên `GPIO47` trong khoảng ngắn mỗi khi thấy khuôn mặt để tăng sáng; kênh RGB `GPIO48` đang tắt mặc định để giảm rủi ro sụt áp. Nếu board thực tế khác biến thể này, sửa các hằng `FACE_LIGHT_*` trong [STM-FaceGuard.ino](/Users/phamhungtien/Documents/STM-FaceGuard/STM-FaceGuard/ESP32-S3/STM-FaceGuard/STM-FaceGuard.ino#L94).
- Nếu muốn cho ESP32-S3 vào cùng Wi-Fi router, sửa 2 hằng trong [STM-FaceGuard.ino](/Users/phamhungtien/Documents/STM-FaceGuard/STM-FaceGuard/ESP32-S3/STM-FaceGuard/STM-FaceGuard.ino#L74):
  - `PREVIEW_STA_SSID`
  - `PREVIEW_STA_PASS`
...
│   └── STM-FaceGuard/
│       └── STM-FaceGuard.ino       # Face recognition + voting + lockout + UART + Wi-Fi preview
│
└── README.md
```
...
| Ngưỡng nhận diện (similarity) | **≥ 0.55** (cosine similarity) |
...
| Watchdog STM32 (IWDG) | ~5.0 giây (LSI ~40kHz, prescaler 64, reload 3124) |
