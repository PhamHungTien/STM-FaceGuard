...
> **Lưu ý nhận dạng board:** module dùng trong project là loại **ESP32-S3-CAM / ESP32-S3 N16R8 CAM** với camera **OV3660**. Hình dáng của nó có thể khá giống các ảnh `ESP32-CAM` thường trên internet, nhưng **pin UART không giống nhau**. Trong project này phải dùng **GPIO1 (TX)** và **GPIO2 (RX)** theo đúng firmware hiện tại.
...
>
> **Không dùng pinout của ESP32-CAM thường (AI-Thinker / OV2640)** cho mục này. Nhiều ảnh mạng ghi `GPIO1 = U0_TXD`, `GPIO3 = U0_RXD`; sơ đồ đó **không áp dụng cho board ESP32-S3-CAM của project này**.
...
| UART nối STM32 | **GPIO1 TX, GPIO2 RX** | Thường thấy GPIO1 TX, GPIO3 RX |
...
| GPIO47 | Flash LED trắng | Tự bật ngắn khi phát hiện khuôn mặt |
...
- Firmware cũng bật `flash LED` trắng trên `GPIO47` trong khoảng ngắn mỗi khi thấy khuôn mặt để tăng sáng. Đây là cấu hình **suy ra từ profile `ESP32-S3-CAM` của Prusa** có pin camera trùng board hiện tại; nếu board bạn là biến thể khác, sửa `FACE_LIGHT_GPIO` hoặc đặt `FACE_LIGHT_ENABLE = 0` trong [STM-FaceGuard.ino](/Users/phamhungtien/Documents/STM-FaceGuard/STM-FaceGuard/ESP32-S3/STM-FaceGuard/STM-FaceGuard.ino#L76).
