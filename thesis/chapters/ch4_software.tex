...
\begin{lstlisting}[style=cstyle, caption={ISR nút nhấn: đặt cờ sự kiện, xử lý thực hiện ở main loop}, label={lst:btn_isr}]
/* Cờ sự kiện toàn cục */
...
\begin{lstlisting}[style=cstyle, caption={Xử lý debounce và gửi lệnh ENROLL trong vòng lặp chính}, label={lst:btn_main}]
...
\begin{lstlisting}[style=cstyle, caption={Hàm mở khóa có thời gian giữ cấu hình được}, label={lst:unlock}]
...
\begin{itemize}
  \item \textbf{\texttt{camera\_task}} (Core 0, Priority~5): Liên tục lấy frame từ OV3660, đưa vào hàng đợi ảnh.
