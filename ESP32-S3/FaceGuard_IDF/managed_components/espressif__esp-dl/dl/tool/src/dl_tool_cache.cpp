...
#include <cstdio>   // fix: printf not declared with picolibc (IDF v6.0)

// Fix for IDF v6.0: CONFIG_TIE728_BOOST no longer exists; ESP32-S3 always
// uses the region-based autoload API (autoload_region_config), so we force
// the TIE728 code path which is the only one compatible with IDF v6.0.
#if CONFIG_IDF_TARGET_ESP32S3 && !defined(CONFIG_TIE728_BOOST)
